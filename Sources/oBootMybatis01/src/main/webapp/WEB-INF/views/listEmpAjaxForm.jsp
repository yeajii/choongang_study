...
<script type="text/javascript" src="js/jquery.js"></script>  <!-- boot 특징으로 static 파일 안에 파일 넣음 -->
<script type="text/javascript">
    function getDeptName(pDeptno) {
		console.log(pDeptno);
		// alert("pDeptno-> " + pDeptno)   // 잘되는지 확인하는 작업 
		
		// 행동강령 :  Ajax로 부서번호 보내고 부서명 받음
		$.ajax(
				{                            // getDeptName : controller에 있는 url 작성 
					url     : "<%=context %>/getDeptName",           
					data    : {deptno : pDeptno},
					dataType: 'text',  				     // 받는 파라미터가 객체라면 JSON으로 적기, String이면 test
					success :function(deptName){ 
						// alert("success ajax Data-> " + deptName);  // 확인하기 위함
	...
	function getDept(pDeptno) {  // pDeptno = 10
		// alert("pDeptno-> " + pDeptno);  // 잘되는지 확인하는 작업 
		$.ajax(
				{
					url      : "sample/sendVO2",
